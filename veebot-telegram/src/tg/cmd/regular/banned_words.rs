...
pub(crate) async fn ban_word(ctx: &tg::Ctx, msg: Message, word: &str) -> Result {


    let created_by = msg.from().unwrap().id;

    ctx.db.tg_chat_banned_words
        .create(msg.chat.id, &word, created_by)
        .await?;
...
pub(crate) async fn banned_words(ctx: &tg::Ctx, msg: Message) -> Result {
    let banned_words: Vec<_> = repo
...
pub(crate) async fn unban_word(ctx: &tg::Ctx, msg: Message, word: String) -> Result {
