...
impl PlatformTrait for Platform {
    ...
    const NAME: &'static str = "Furbooru";

    fn new(params: PlatformParams<Config>) -> Self {
        Self {
            tools: Derpitools::new(params, DerpiPlatformKind::Furbooru),
        }
    }
    ...
    fn parse_query(query: &str) -> ParseQueryResult<MediaId> {
        let (_, host, id) = parse_with_regexes!(
            query,
            r"(furbooru.org(?:/images)?)/(\d+)",
            r"(furrycdn.org/img)/\d+/\d+/\d+/(\d+)",
            r"(furrycdn.org/img/(?:view|download))/\d+/\d+/\d+/(\d+)",
        )?;
