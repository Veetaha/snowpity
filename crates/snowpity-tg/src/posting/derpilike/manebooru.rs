...
impl PlatformTrait for Platform {
    ...
    const NAME: &'static str = "Manebooru";

    fn new(params: PlatformParams<Config>) -> Self {
        Self {
            tools: Derpitools::new(params, DerpiPlatformKind::Manebooru),
        }
    }
    ...
    fn parse_query(query: &str) -> ParseQueryResult<MediaId> {
        let (_, host, id) = parse_with_regexes!(
            query,
            r"(manebooru.art(?:/images)?)/(\d+)",
            r"(static.manebooru.art/img)/\d+/\d+/\d+/(\d+)",
            r"(static.manebooru.art/img/(?:view|download))/\d+/\d+/\d+/(\d+)",
        )?;
