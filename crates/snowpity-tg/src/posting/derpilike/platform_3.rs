...
mod derpibooru {
    ...
    use crate::posting::derpilike::api::{self, MediaId};
    use crate::posting::derpilike::db;
    use crate::posting::derpilike::platform_3::Derpitools;
    ...
    use super::DerpiPlatformKind;

    struct Derpibooru {
        tools: Derpitools,
    }

    impl PlatformTypes for Derpibooru {
        type PostId = MediaId;
        type BlobId = ();
    ...
    #[async_trait]
    impl PlatformTrait for Derpibooru {
        type Config = Config;
            ...
            Self {
                tools: Derpitools {
                    api: api::Client::new(
                        params.config,
                        params.http,
                        DerpiPlatformKind::Derpibooru,
                    ),
                    db: db::BlobCacheRepo::new(params.db, "derpibooru"),
                },
            }
        }
        ...
        async fn get_post(&self, media: MediaId) -> Result<Post<Self>> {
            ...
            let repr = crate::posting::derpilike::platform_3::best_tg_reprs(&media).map_collect(
                |(download_url, kind)| {
                    BlobRepr {
            ...
            Ok(Post {
                base: BasePost {
                    id: media.id,
                    authors,
                    web_url: media.id.to_webpage_url(DerpiPlatformKind::Derpibooru),
                    safety,
                },
                blobs: vec![blob],
            })
        ...
        async fn set_cached_blob(&self, media: MediaId, blob: CachedBlobId<Self>) -> Result {
            self.tools.db.set(media, blob.tg_file).await
        }
    }
}
    ...
    impl PlatformTrait for Platform {
        ...
        async fn get_post(&self, media: MediaId) -> Result<Post<Self>> {
            let media = self
                ...
                .api
                .get_media(media)
                .instrument(info_span!("Fetching media meta from Ponerpics"))
                .await?;
            ...
            Ok(Post {
                base: BasePost {
                    id: media.id,
                    authors,
                    web_url: media.id.to_webpage_url(DerpiPlatformKind::Ponerpics),
                    safety,
                },
                blobs: vec![blob],
            })
